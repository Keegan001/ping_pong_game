...
capture.set(3,1280)             ##width denoted by 3 in documentation
capture.set(4,720)              ##height denoted by 2 in documentation

background =cv.imread('CVproject\Resources\Background.png')
print(background)
#background= cv.resize(background,(1280,720))
ball=cv.imread('CVproject\Resources\Ball.png',cv.IMREAD_UNCHANGED)
bat1=cv.imread('CVproject\Resources\Bat1.png',cv.IMREAD_UNCHANGED)
bat2=cv.imread('CVproject\Resources\Bat2.png',cv.IMREAD_UNCHANGED)
gameOver=cv.imread('CVproject\Resources\GameOver.png')

#initializing
position=[100,100]              ##try and error se aaya 100,100(coordinates hai)
isOver=False
speedX,speedY=10,10 
score=[0,0]
while True:
